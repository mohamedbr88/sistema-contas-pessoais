...
export function telaViagem(){
  ...
  return `<div class="card"><div class="card-hd"><h3>${esc(v.destino||v.nome)} · ${dd(v.inicio)} a ${dd(v.fim)} · ${dias} dias</h3>
    ...
    <div class="kpis">
      <div class="kpi"><span>Hospedagem</span><b>${VE(th,mo)}</b><small>${noites} noites · ${VE(th/(noites||1),mo)}/noite${VC(th,mo,cxv)?' · '+VC(th,mo,cxv):''}</small></div>
      <div class="kpi"><span>Gastos do dia</span><b>${VE(tg,mo)}</b><small>${v.gastos.length} lançamentos${VC(tg,mo,cxv)?' · '+VC(tg,mo,cxv):''}</small></div>
      <div class="kpi"><span>Total da viagem</span><b style="color:var(--cofre)">${VE(th+tg,mo)}</b><small>${dias} dias${VC(th+tg,mo,cxv)?' · '+VC(th+tg,mo,cxv):''}</small></div>
      <div class="kpi"><span>Por dia</span><b>${VE((th+tg)/dias,mo)}</b><small>hosp ${VE(th/(noites||1),mo)} + dia ${VE(tg/dias,mo)}</small></div>
    </div></div>
    <div class="grid2">
      <div class="card"><div class="card-hd"><h3>Onde foi o dinheiro</h3></div>
        <div style="padding:8px 0 12px">${!porCat.length?`<div class="vazio" style="padding:26px 16px">Nada gasto ainda.<br>As barras aparecem no primeiro lançamento.</div>`:''}${porCat.map(x=>`<div class="barra"><div class="lab">${esc(x.k)}</div>
          ...
          <div class="v">${VE(x.v,mo)}</div></div>`).join('')}</div></div>
      <div class="card"><div class="card-hd"><h3>Gasto por dia</h3><div class="dir" style="font-size:11px;color:var(--ink-2)">só o dia a dia, sem hospedagem</div></div>
        <div style="padding:10px 14px;display:flex;gap:2px;align-items:flex-end;height:120px">
          ...
          ${Object.keys(porDia).sort().map(d=>`<div title="${dd(d)} · ${VE(porDia[d],mo)}" style="flex:1;background:var(--cofre);opacity:.75;border-radius:2px 2px 0 0;height:${porDia[d]/maxD*100}%"></div>`).join('')}
        </div><div style="padding:0 14px 10px;font-size:10px;color:var(--ink-2);display:flex;justify-content:space-between">
          <span class="mono">${dd(v.inicio)}</span><span class="mono">maior dia: ${VE(maxD,mo)}</span><span class="mono">${dd(v.fim)}</span></div></div>
    </div>
    <div class="card"><div class="card-hd"><h3>Hospedagem</h3>
      <div class="dir"><span class="mono" style="font-size:12px;color:var(--ink-2)">${VE(th,mo)}</span><button class="btn-2" id="addH">+ Estadia</button></div></div>
      <table class="tmes"><thead><tr><th>Entrada</th><th>Hotel</th><th>Cidade</th><th class="num">Noites</th>
        <th class="num">Valor (${MOEDAS[mo].n})</th><th class="num">Por noite</th><th></th><th></th></tr></thead>
      <tbody>${!v.hosp.length?`<tr><td colspan="8" class="vazio" style="padding:22px">Nenhuma hospedagem ainda. Use <b style="color:var(--ink)">+ Estadia</b>.</td></tr>`:''}${v.hosp.slice().sort((a,b)=>a.data.localeCompare(b.data)).map((h,i)=>`<tr class="lin" style="--c:${cor('Hospedagem')}">
        ...
        <td class="num">${h.noites}</td><td class="num" style="font-weight:600">${VE(h.valor,mo)}${VC(h.valor,mo,cxv)?`<div style="font-size:10px;font-weight:400;color:var(--ink-2)">${VC(h.valor,mo,cxv)}</div>`:''}</td>
        <td class="num" style="font-size:12px;color:var(--ink-2)">${VE(h.valor/(h.noites||1),mo)}</td><td></td>
        <td style="text-align:right"><button class="acao" data-eh="${h.data}">editar</button></td></tr>`).join('')}</tbody>
      <tfoot><tr class="tot"><td colspan="3">${v.hosp.length} estadias</td><td class="num">${noites}</td>
        <td class="num">${VE(th,mo)}</td><td class="num">${VE(th/(noites||1),mo)}</td><td colspan="2"></td></tr></tfoot></table></div>
    <div class="card"><div class="card-hd"><h3>Gastos do dia</h3>
      <div class="dir"><span class="mono" style="font-size:12px;color:var(--ink-2)">${VE(tg,mo)}</span><button class="btn" id="addG">+ Gasto</button></div></div>
      <div style="max-height:520px;overflow:auto"><table class="tmes"><thead><tr><th>Dia</th><th>O quê</th><th>Categoria</th><th>Pgto</th>
        <th class="num">Valor (${MOEDAS[mo].n})</th><th class="num">Em ${MOEDAS[S.moeda||'BRL'].n}</th><th></th><th></th></tr></thead>
      <tbody>${!v.gastos.length?`<tr><td colspan="8" class="vazio" style="padding:22px">Nenhum gasto ainda. Use <b style="color:var(--ink)">+ Gasto</b>.</td></tr>`:''}${v.gastos.slice().sort((a,b)=>a.data.localeCompare(b.data)).map((g,i)=>`<tr class="lin" style="--c:${cor(g.cat)}">
        ...
        <td class="num" style="font-weight:600">${VE(g.valor,mo)}</td>
        <td class="num" style="font-size:12px;color:var(--ink-2)">${VC(g.valor,mo,cxv)||'—'}</td><td></td>
...
export function formGasto(id){
  ...
  ], {...g, moeda: rot(g.moeda || v.moeda || 'EUR')}, async o=>{
    o.moeda = Object.keys(MOEDAS).find(m => rot(m) === o.moeda) || v.moeda;
